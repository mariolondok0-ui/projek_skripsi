...
if (isset($_GET['delete'])) {
    ...
    setAlert('success', 'Data kas keluar berhasil dihapus.');
    redirect(APP_URL . '/admin/kas-keluar.php');
}
  ...
  <div class="admin-content">
    ...
    <div style="display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:16px;margin-bottom:16px">
      <div style="display:flex;gap:16px;flex-wrap:wrap">
        <div style="background:var(--bg-card);padding:14px 20px;border-radius:var(--radius);box-shadow:var(--shadow-sm);display:flex;align-items:center;gap:10px">
          ...
          <div><div style="font-size:.72rem;color:var(--text-muted)">Total Pengeluaran Bulan Ini</div>
          <div style="font-weight:800;color:var(--danger)"><?= formatRupiah($summary['total']) ?></div></div>
        </div>
        <div style="background:var(--bg-card);padding:14px 20px;border-radius:var(--radius);box-shadow:var(--shadow-sm);display:flex;align-items:center;gap:10px">
    ...
    <div class="table-wrapper animate-fadeIn">
      <table class="table table-striped">
        ...
        <?php if ($rows->num_rows): ?>
        <tfoot>
          <tr><td colspan="4" class="text-right">Total:</td><td class="text-right text-danger">- <?= number_format($summary['total'],0,',','.') ?></td><td></td></tr>
        </tfoot>
        <?php endif; ?>
      </table>
    </div>
