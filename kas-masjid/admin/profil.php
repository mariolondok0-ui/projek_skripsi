...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        if (empty($nama)) {
            ...
            if (isset($_FILES['foto']) && $_FILES['foto']['error'] === UPLOAD_ERR_OK) {
                ...
                if (!in_array($file_ext, $allowed_ext)) {
                    ...
                    redirect(APP_URL.'/admin/profil.php?tab=profil');
                    exit;
    ...
    if ($action === 'update_email') {
        ...
        redirect(APP_URL.'/admin/profil.php?tab=email');
        exit;
    }
    ...
    if ($action === 'update_password') {
        ...
        redirect(APP_URL.'/admin/profil.php?tab=password');
        exit;
    }
}
...
.profile-hero::before {
  content: ''; position: absolute; top: -40px; right: -40px; width: 160px; height: 160px; background: rgba(255,255,255,.07); border-radius: 50%;
}
.profile-hero::after {
  content: ''; position: absolute; bottom: -30px; left: -30px; width: 120px; height: 120px; background: rgba(201,168,76,.12); border-radius: 50%;
}

.avatar-wrapper { position: relative; display: inline-block; flex-shrink: 0; }
.profile-avatar {
  width: 80px; height: 80px; border-radius: 50%; background: rgba(255,255,255,.2); border: 3px solid rgba(255,255,255,.4);
...
.btn-edit-avatar {
  position: absolute; bottom: 0; right: 0; width: 28px; height: 28px; background: #ffffff; color: #1e6eb5;
...
.profile-stat {
  ...
  padding: 12px 16px; text-align: center; flex: 1; min-width: 100px; backdrop-filter: blur(4px);
}
...
/* NAV TABS KUSTOM */
.settings-tabs {
  display: flex; gap: 8px; background: #e2e8f0; padding: 5px; border-radius: var(--radius-lg); margin-bottom: 20px;
}
.settings-tab-btn {
  flex: 1; padding: 10px 15px; text-align: center; border-radius: var(--radius); font-size: 0.85rem; font-weight: 600;
  color: var(--text-secondary); background: transparent; border: none; cursor: pointer; transition: all 0.2s;
  display: flex; align-items: center; justify-content: center; gap: 8px; text-decoration: none;
}
.settings-tab-btn:hover { color: var(--primary); }
.settings-tab-btn.active { background: #ffffff; color: var(--primary); box-shadow: var(--shadow-sm); }
...
/* Form Card */
.form-card {
  background: var(--bg-card); border-radius: var(--radius-xl); box-shadow: var(--shadow); border: 1px solid var(--border-light); overflow: hidden; margin-bottom: 20px;
}
.form-card-header {
  padding: 18px 22px; border-bottom: 1px solid var(--border-light); display: flex; align-items: center; gap: 10px; background: var(--bg-main);
}
.form-card-header .fch-icon {
  width: 36px; height: 36px; border-radius: var(--radius-sm); background: var(--primary);
...
.toggle-pw {
  position: absolute; right: 6px; top: 50%; transform: translateY(-50%); background: none; border: none; color: var(--text-muted); cursor: pointer; font-size: 1rem; padding: 10px 12px; z-index: 2; border-radius: 8px; transition: background 0.2s;
}
...
@media (max-width: 640px) {
  ...
  .settings-tabs { flex-direction: column; gap: 4px; }
}
</style>
</head>
<body>
...
<div class="admin-main">
  ...
  <div class="admin-content" style="max-width:680px">
    ...
    <!-- NAV TABS KHUSUS MENU -->
    <div class="settings-tabs">
      <button class="settings-tab-btn <?= $active_tab=='profil'?'active':'' ?>" onclick="switchTab('profil', this)">
        <i class="fas fa-user-edit"></i> Edit Profil
      </button>
      <button class="settings-tab-btn <?= $active_tab=='email'?'active':'' ?>" onclick="switchTab('email', this)">
        <i class="fas fa-envelope"></i> Ubah Email
      </button>
      <button class="settings-tab-btn <?= $active_tab=='password'?'active':'' ?>" onclick="switchTab('password', this)">
        <i class="fas fa-key"></i> Ubah Password
      </button>
    </div>

    <!-- TAB 1: EDIT PROFIL -->
    <div class="tab-pane <?= $active_tab=='profil'?'active':'' ?>" id="tab-profil">
      <div class="form-card">
        ...
        <div class="form-card-body">
          <form method="POST" enctype="multipart/form-data">
            ...
            <button type="submit" class="btn btn-primary w-100" style="justify-content:center;height:48px">
    ...
    <!-- TAB 2: UBAH EMAIL -->
    <div class="tab-pane <?= $active_tab=='email'?'active':'' ?>" id="tab-email">
      <div class="form-card">
        <div class="form-card-header">
    ...
    <div class="tab-pane <?= $active_tab=='password'?'active':'' ?>" id="tab-password">
          ...
          <form method="POST" autocomplete="off">
            ...
            <div class="form-group">
              ...
              <div class="input-group" style="position:relative">
                ...
                <input type="password" name="password_lama" id="pwLama" class="form-control" placeholder="Masukkan password saat ini" required style="padding-right:50px">
            ...
            <div class="form-group">
              ...
              <div class="input-group" style="position:relative">
                ...
                <input type="password" name="password_baru" id="pwBaru" class="form-control" placeholder="Minimal 6 karakter" required minlength="6" style="padding-right:50px">
            ...
            <div class="form-group" style="margin-bottom:24px">
              ...
              <div class="input-group" style="position:relative">
                ...
                <input type="password" name="password_ulang" id="pwUlang" class="form-control" placeholder="Ulangi password baru" required style="padding-right:50px">
      ...
      <div class="form-card">
        ...
        <div class="form-card-body">
          <div class="info-grid">
            <div>
              <div style="font-size:.85rem;font-weight:700;color:var(--text-primary);"><i class="fas fa-clipboard-check" style="color:var(--primary);margin-right:6px"></i>Syarat Minimal</div>
            ...
            <div>
              <div style="font-size:.85rem;font-weight:700;color:var(--text-primary);margin-bottom:10px"><i class="fas fa-lightbulb" style="color:var(--secondary);margin-right:6px"></i>Tips Ekstra Aman</div>
    ...
    <!-- TOMBOL KEMBALI -->
    <a href="<?= APP_URL ?>/admin/dashboard.php" class="btn" style="background:var(--primary);color:#fff;border:none;margin-top:10px">
...
function switchTab(tabName, btnElement) {
    ...
    // Update URL tanpa reload halaman agar bersih
