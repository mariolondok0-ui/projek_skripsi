...
@keyframes slideInChart{from{opacity:0;transform:translateX(50px) scale(.97)}to{opacity:1;transform:translateX(0) scale(1)}}
...
.pie-legend-dot{width:14px;height:14px;border-radius:4px;flex-shrink:0;display:inline-block}
</style>
</head>
<body>
    ...
    <div class="grid-3" style="align-items:start">
      <div class="saldo-card animate-fadeIn">
        ...
        <div style="margin-top:20px;display:flex;gap:10px;justify-content:center;flex-wrap:wrap">
          <a href="laporan-publik.php" class="btn btn-sm" style="background:rgba(255,255,255,.18);color:#fff;border:1px solid rgba(255,255,255,.3)"><i class="fas fa-file-alt"></i> Laporan</a>
          <a href="grafik-publik.php" class="btn btn-sm" style="background:rgba(255,255,255,.18);color:#fff;border:1px solid rgba(255,255,255,.3)"><i class="fas fa-chart-pie"></i> Grafik</a>
        </div>
      </div>
      ...
      <div class="card animate-fadeIn delay-3" style="padding:28px">
        <div style="text-align:center;margin-bottom:18px">
          <div style="width:60px;height:60px;background:linear-gradient(135deg,var(--primary),var(--accent));border-radius:16px;display:flex;align-items:center;justify-content:center;font-size:1.6rem;color:#fff;margin:0 auto 12px;box-shadow:0 6px 18px rgba(26,122,74,.3)"><i class="fas fa-mosque"></i></div>
          <h3 style="font-size:.95rem;font-weight:800;color:var(--text-primary)"><?= MASJID_NAME ?></h3>
          <p style="font-size:.75rem;color:var(--text-muted);margin-top:4px;line-height:1.6"><?= MASJID_ALAMAT ?></p>
        </div>
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px">
          <div style="text-align:center;padding:12px;background:var(--bg-main);border-radius:var(--radius)"><div style="font-size:1.4rem;font-weight:800;color:var(--primary)"><?= $total_trx ?></div><div style="font-size:.7rem;color:var(--text-muted)">Total Transaksi</div></div>
          <div style="text-align:center;padding:12px;background:var(--bg-main);border-radius:var(--radius)"><div style="font-size:1.4rem;font-weight:800;color:var(--secondary)"><?= $trx_bln ?></div><div style="font-size:.7rem;color:var(--text-muted)">Bulan Ini</div></div>
        </div>
      </div>
    </div>
  </div>
...
<!-- GRAFIK SLIDESHOW -->
<section class="section" id="grafik-section" style="padding-top:16px;padding-bottom:52px;background:linear-gradient(180deg,#f0f4f0,#e8f5ee)">
  <div class="container">
    ...
    <div class="card animate-fadeIn" style="padding:28px">
      ...
      <div style="display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:12px;margin-bottom:20px">
        <div>
          <div style="display:inline-flex;align-items:center;gap:8px;background:var(--primary);color:#fff;padding:6px 16px;border-radius:99px;font-size:.82rem;font-weight:700"><i id="slideIcon" class="fas fa-chart-bar"></i> <span id="slideTitle">Pemasukan vs Pengeluaran</span></div>
          <div style="font-size:.8rem;color:var(--text-muted);margin-top:8px" id="slideDesc">Perbandingan pemasukan dan pengeluaran 6 bulan terakhir</div>
        </div>
        <div style="display:flex;align-items:center;gap:12px">
          ...
          <div style="display:flex;gap:8px">
            ...
            <button class="slide-nav-btn" onclick="togglePause()"><i class="fas fa-pause" id="pauseIcon"></i></button>
      ...
      <div class="grafik-slide" id="slide-2">
        <?php if(count($pid)):?><div class="chart-pie-wrap"><div style="position:relative;width:260px;height:260px;flex-shrink:0"><canvas id="pieIncome"></canvas></div><div id="legIncome" style="display:flex;flex-direction:column;gap:10px;min-width:200px;max-width:320px"></div></div><?php else:?><div class="empty-state"><div class="es-icon"><i class="fas fa-chart-pie"></i></div><h3>Belum ada data pemasukan <?=$tahun?></h3></div><?php endif;?>
      </div>
      <div class="grafik-slide" id="slide-3">
        <?php if(count($ped)):?><div class="chart-pie-wrap"><div style="position:relative;width:260px;height:260px;flex-shrink:0"><canvas id="pieExpense"></canvas></div><div id="legExpense" style="display:flex;flex-direction:column;gap:10px;min-width:200px;max-width:320px"></div></div><?php else:?><div class="empty-state"><div class="es-icon"><i class="fas fa-chart-pie"></i></div><h3>Belum ada data pengeluaran <?=$tahun?></h3></div><?php endif;?>
      </div>
  ...
  <div class="container">
    ...
    <div class="grid-3">
      <?php $feats=[
        ...
        ['fas fa-shield-alt','green','Data Terverifikasi','Setiap transaksi dicatat oleh pengurus yang bertanggung jawab.'],
        ['fas fa-clock','gold','Real-time Update','Informasi saldo diperbarui secara langsung tanpa perlu reload.'],
        ['fas fa-mobile-alt','blue','Akses Kapan Saja','Dapat diakses dari perangkat apapun tanpa menginstal aplikasi.'],
      ]; foreach($feats as $i=>[$ico,$col,$ttl,$dsc]): ?>
      <div class="card animate-fadeIn delay-<?=($i%3)+1?>" style="padding:28px">
        <div style="width:52px;height:52px;border-radius:14px;background:<?=$col=='green'?'rgba(26,122,74,.1)':($col=='gold'?'rgba(201,168,76,.1)':'rgba(59,130,246,.1)')?>;display:flex;align-items:center;justify-content:center;font-size:1.3rem;color:<?=$col=='green'?'var(--primary)':($col=='gold'?'var(--secondary)':'var(--info)')?>;margin-bottom:16px"><i class="<?=$ico?>"></i></div>
        <h3 style="font-size:.95rem;font-weight:700;margin-bottom:8px"><?=$ttl?></h3>
        <p style="font-size:.85rem;color:var(--text-secondary);line-height:1.7"><?=$dsc?></p>
      </div>
      <?php endforeach; ?>
    </div>
  </div>
...
new Chart(document.getElementById('barChart').getContext('2d'),{
  ...
  options:{responsive:true,maintainAspectRatio:false,interaction:{mode:'index',intersect:false},
    plugins:{legend:{position:'bottom',labels:{padding:20,font:{size:12},usePointStyle:true}},tooltip:{callbacks:{label:c=>` ${c.dataset.label}: ${fmtRp(c.raw)}`}}},
...
new Chart(document.getElementById('lineChart').getContext('2d'),{
  ...
  data:{labels:<?=json_encode($line_labels)?>,datasets:[{
    ...
    pointBackgroundColor:'#1a7a4a',pointBorderColor:'#fff',pointBorderWidth:2,pointRadius:6,pointHoverRadius:9
  }]},
...
const INFO=[
  ...
  {icon:'fas fa-chart-line',title:'Saldo Kumulatif',desc:'Tren pertumbuhan saldo kas masjid tahun <?=$tahun?>'},
  {icon:'fas fa-chart-pie',title:'Proporsi Pemasukan',desc:'Distribusi sumber pemasukan berdasarkan kategori tahun <?=$tahun?>'},
  {icon:'fas fa-chart-pie',title:'Proporsi Pengeluaran',desc:'Distribusi pengeluaran berdasarkan kategori tahun <?=$tahun?>'},
];
...
function togglePause(){
  paused=!paused;const ico=document.getElementById('pauseIcon');
  ...
  else{ico.className='fas fa-pause';startAuto();resetProg();}
}
startAuto();resetProg();
